...
# -----------------------------------------------------------------------------
# 3. 데이터 영구 저장소 초기화 (앞으로 데이터가 절대 안 사라집니다)
# -----------------------------------------------------------------------------
...
with tab_dashboard:
    ...
    # 예상 달성일 안내
    monthly_dca_monthly = total_daily_dca_krw * 30.0 # 월 적립액 추정
    if total_monthly_div_krw < goal_final:
        # 간단한 복리 성장 시뮬레이션 (월 적립액 + 배당 재투자 가정)
        remaining_gap = goal_final - total_monthly_div_krw
        effective_monthly_add = monthly_dca_monthly * 0.005 # 자산 대비 월 배당 증가 추정치
    ...
    with col_p1:
        ...
        if not div_df.empty and div_df['금액(원)'].sum() > 0:
            fig_pie = px.pie(div_df, values='금액(원)', names='종목코드', hole=0.4, color_discrete_sequence=px.colors.sequential.Greens_r)
    ...
    with col_p2:
        ...
        table_df['월 평균 배당금'] = (table_df['연간세후배udan'] if '연간세후udan' in table_df else table_df['연간세후배당(USD)'] * live_rate / 12.0).apply(format_krw)
...
with tab_settings:
    ...
    edited_df = st.data_editor(
        st.session_state['portfolio_permanent'],
        num_rows="dynamic",
